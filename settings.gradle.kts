pluginManagement {
    repositories {
        google {
            content {
                includeGroupByRegex("com\\.android.*")
                includeGroupByRegex("com\\.google.*")
                includeGroupByRegex("androidx.*")
            }
        }
        mavenCentral()
        gradlePluginPortal()
    }
}
plugins {
    id("org.gradle.toolchains.foojay-resolver-convention") version "1.0.0"
}
dependencyResolutionManagement {
    repositoriesMode.set(RepositoriesMode.FAIL_ON_PROJECT_REPOS)
    repositories {
        google()
        mavenCentral()
        // Required by MSAL (OneDrive sign-in): its "common" internal
        // module depends on com.microsoft.device.display:display-mask,
        // which Microsoft only publishes here, not on Maven Central --
        // see https://learn.microsoft.com/entra/identity-platform/tutorial-mobile-app-android-prepare-app
        maven {
            url = uri("https://pkgs.dev.azure.com/MicrosoftDeviceSDK/DuoSDK-Public/_packaging/Duo-SDK-Feed/maven/v1")
            name = "Duo-SDK-Feed"
        }
        // Microsoft's own Maven feed — hosts com.microsoft.device.display:display-mask,
        // a transitive dependency of MSAL (com.microsoft.identity:common) for Surface
        // Duo/foldable support. Not on Maven Central or Google's Maven; required for
        // the OneDrive connector (punch item #8) to resolve.
        maven {
            url = uri("https://pkgs.dev.azure.com/MicrosoftDeviceSDK/DuoSDK-Public/_packaging/Duo-SDK-Feed/maven/v1")
        }
    }
}

rootProject.name = "MedRecall+"
include(":app")
